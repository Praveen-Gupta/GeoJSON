...
namespace MIVector {
	namespace util {
		...
		{
			return MIVector::util::apply_visitor(detail::dynamic_caster<T>(), v);
		}
		...
		{
			return MIVector::util::apply_visitor(detail::dynamic_caster<const T>(), v);
		}

		template <class T, class V>
		T& static_variant_cast(V& v)
		{
			return MIVector::util::apply_visitor(detail::static_caster<T>(), v);
		}

		template <class T, class V>
		const T& static_variant_cast(const V& v)
		{
			return MIVector::util::apply_visitor(detail::static_caster<const T>(), v);
		}
	}
}
