#ifndef MIVECTOR_UTIL_VARIANT_HPP
#define MIVECTOR_UTIL_VARIANT_HPP
...
#include <MIVector/recursive_wrapper.hpp>
#include <MIVector/variant_visitor.hpp>

// clang-format off
// [[deprecated]] is only available in C++14, use this for the time being
#if __cplusplus <= 201103L
# ifdef __GNUC__
#  define MIVECTOR_VARIANT_DEPRECATED __attribute__((deprecated))
# elif defined(_MSC_VER)
#  define MIVECTOR_VARIANT_DEPRECATED __declspec(deprecated)
# else
#  define MIVECTOR_VARIANT_DEPRECATED
# endif
#else
#  define MIVECTOR_VARIANT_DEPRECATED [[deprecated]]
#endif
...
namespace MIVector {
	namespace util {
		...
		//     See https://github.com/MIVector/variant/issues/48 for details.
		...
		struct /*MIVECTOR_VARIANT_DEPRECATED*/ static_visitor
		{
			using result_type = R;
...
#if !defined(MIVECTOR_VARIANT_MINIMIZE_SIZE)
		using type_index_t = unsigned int;
#else
#if defined(MIVECTOR_VARIANT_OPTIMIZE_FOR_SPEED)
		...
		public:
			...
			// Use which() instead.
			MIVECTOR_VARIANT_DEPRECATED VARIANT_INLINE type_index_t get_type_index() const
			{
				return type_index;
			}
			...
			auto VARIANT_INLINE match(Fs&&... fs) const
				-> decltype(variant::visit(*this, ::MIVector::util::make_visitor(std::forward<Fs>(fs)...)))
			{
				return variant::visit(*this, ::MIVector::util::make_visitor(std::forward<Fs>(fs)...));
			}
			// non-const
			template <typename... Fs>
...
namespace std {
	template <typename... Types>
	struct hash< ::MIVector::util::variant<Types...>> {
		std::size_t operator()(const ::MIVector::util::variant<Types...>& v) const noexcept
		{
			return ::MIVector::util::apply_visitor(::MIVector::util::detail::hasher{}, v);
		}
	};
}

#endif // MIVECTOR_UTIL_VARIANT_HPP
