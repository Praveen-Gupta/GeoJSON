...
static void testPoint(bool use_convert)
{
	...
	//std::ofstream out("test/fixtures/output.json");
	//out << str;
	//out.close();
}
...
static void testGeometryCollection(bool use_convert)
{
	...
	std::string str = writeGeoJSON(data, use_convert);
	std::string outfile = "test/fixtures/output.json";
...
static void testFeature(bool use_convert) {
	...
	// Explicit recursive_wrapper as a workaround for https://github.com/MIVector/variant/issues/102
